...
void LinearSearchJudul(int &n, RankingMusik *musik, string SearchJudul ){
    bool found = false;
    for (int i = 0; i < n; i++){
        if ((musik + i)-> judulLagu == SearchJudul){
            cout << "Lagu ditemukan di posisi " << (musik + i)-> posisi << " dengan alamat memori =  " << (musik + i) << endl;
            cout << "---------------------------------------------------\n";
            found = true;
            break;
        }
...
void cariLagu(int &n, RankingMusik musik[]){
    ...
    LinearSearchJudul(n, musik, cariJudul);
}
...
void binarySearchHarga(PaketLangganan *paket, int n1, int targetHarga){
    int low = 0, high = n1 - 1;
    while (low <= high){
        int mid = low + (high - low)/2;
        if ((paket + mid)->hargaPaket == targetHarga){
            cout << "Paket dengan harga " << targetHarga << " ditemukan: " << (paket + mid)->namaPaket << endl;
            return;
        }
        else if ((paket + mid)->hargaPaket < targetHarga){
            low = mid + 1;
        }
        else {
            high = mid - 1;
    ...
    cout << "Paket dengan harga " << targetHarga << " tidak ditemukan\n";
}
void Belipaket (PaketLangganan paket[], int &n1){
    string IDbeli;
    int targetHarga;
    cout << "===============================================\n";
